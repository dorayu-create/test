...
const GoalRow: React.FC<GoalRowProps> = ({ goal, yearProgress, monthDays, onLog, onEdit, isViewMode = false }) => {
  ...
  return (
    <div className={`grid grid-cols-[100px_repeat(31,32px)] md:grid-cols-[80px_200px_70px_70px_70px_70px_180px_repeat(31,32px)] items-stretch border-b border-gray-100 group bg-white hover:bg-slate-50/50`}>
      {/* KR - Desktop Only */}
      ...
      {/* Title - Sticky with high Z-index to overlay grid content */}
      <div 
        className={`p-3 border-r border-gray-100 flex items-center justify-between sticky left-0 z-20 bg-white group-hover:bg-blue-50/40 shadow-[2px_0_5px_rgba(0,0,0,0.05)] transition-colors ${isViewMode ? 'cursor-default' : 'cursor-pointer'}`} 
      ...
      <div className="hidden md:flex p-3 border-r border-gray-100 items-center space-x-2">
        <div className="flex-grow bg-slate-100 h-5 rounded overflow-hidden border border-slate-200">
      ...
      {/* Date Checks - Directly integrated into the main grid */}
      {monthDays.map(d => {
        ...
        return (
          <button
            ...
            className={`w-8 min-h-[48px] border-r border-gray-50 flex items-center justify-center transition-all ${
